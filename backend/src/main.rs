            ...
            match action {
                ...
                "set_pv_mode" => {
                    if let Some(mode_str) = cmd.get("mode").and_then(|v| v.as_str()) {
                        ...
                        let pv = state.pv_dcdc.lock().expect("Failed to lock pv_dcdc");
...
async fn main() {
    ...
    let system_state = Arc::new(initialize_system());

    // Perform health checks
    if !perform_health_checks(&system_state) {
        println!("System health check failed. Exiting.");
        return;
    }

    println!("All devices healthy. Starting service...");
    ...
    let state_clone = system_state.clone();
    let data_tx_clone = data_tx.clone();
    thread::spawn(move || data_collection_thread(state_clone, data_tx_clone));
...
fn initialize_system() -> Arc<SystemState> {
    // Initialize devices and drivers
    // TODO: Implement device initialization with configuration
    let charger = Arc::new(Mutex::new(charger::ChargerDevice::new("charger1".to_string(), "can0").expect("Failed to initialize charger device")));
    let battery = Arc::new(Mutex::new(bms::BatteryDevice::new("battery1".to_string(), "can0").expect("Failed to initialize battery device")));
    let pcs = Arc::new(Mutex::new(pcs::PcsDevice::new("pcs1".to_string(), "127.0.0.1", 502).expect("Failed to initialize PCS device")));
    let pv_dcdc = Arc::new(Mutex::new(pv_dcdc::PvDcdcDevice::new("pv_dcdc1".to_string())));
    let genset = Arc::new(Mutex::new(genset::GensetDevice::new("genset1".to_string(), "127.0.0.1", 503).expect("Failed to initialize genset device")));
    let can_driver = Arc::new(Mutex::new(can::CanDriver::new("can0").expect("Failed to initialize CAN driver")));
    let modbus_driver = Arc::new(Mutex::new(modbus::ModbusDriver::new()));
    let gps_4g_driver = Arc::new(Mutex::new(gps_4g::Gps4gDriver::new()));
    ...
    ems_controller.add_pv_device(pv_dcdc.clone()).expect("Failed to add PV device");
    ems_controller.add_battery_device(battery.clone()).expect("Failed to add battery device");
    ems_controller.add_pcs_device(pcs.clone()).expect("Failed to add PCS device");
    ems_controller.add_genset_device(genset.clone()).expect("Failed to add genset device");
    ems_controller.add_charger_device(charger.clone()).expect("Failed to add charger device");
...
fn data_collection_thread(state: Arc<SystemState>, data_tx: mpsc::Sender<String>) {
    loop {
        ...
        // Use Rayon thread pool for parallel, non-blocking data collection from devices
        let (charger_data, battery_data, pv_data, pcs_data, genset_data) = rayon::scope(|s| {
            let charger_data = s.spawn(|_| {
            ...
            (
                charger_data.join().unwrap_or(serde_json::Value::Null),
                battery_data.join().unwrap_or(serde_json::Value::Null),
                pv_data.join().unwrap_or(serde_json::Value::Null),
                pcs_data.join().unwrap_or(serde_json::Value::Null),
                genset_data.join().unwrap_or(serde_json::Value::Null),
            )
        });

        // Send collected data with timestamp to sending thread
        let data_points = vec![charger_data, battery_data, pv_data, pcs_data, genset_data];
        ...
        // No fixed frequency requirement - data collection happens as fast as possible
